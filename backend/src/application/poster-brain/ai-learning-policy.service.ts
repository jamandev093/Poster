...
export interface PosterBrainAiLearningPolicyEnvironment {
  readonly AUTO_LEARNING_ENABLED?: string | undefined;
  readonly TRAINING_MIN_EVENTS?: string | undefined;
  readonly MODEL_PROMOTION_REQUIRES_EVAL_PASS?: string | undefined;
}
...
export function createPosterBrainAiLearningPolicy(
  environment: PosterBrainAiLearningPolicyEnvironment = {}
  ...
  const activationState: PosterBrainAiLearningActivationState =
    autoLearningEnabled
      ? "armed"
      : "disabled";

  const minimumEventThresholdMet = (
    observedEventCount: number
  ): boolean => {
  ...
  const canStartTraining = (
    ...
    return autoLearningEnabled &&
      minimumEventThresholdMet(
        observedEventCount
      );
  };

  const canPromoteModel = (
    evaluationPassed: boolean
  ): boolean => {
  ...
  const policy: PosterBrainAiLearningPolicy = {
    autoLearningEnabled,
    trainingMinEvents,
